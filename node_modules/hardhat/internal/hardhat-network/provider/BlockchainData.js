...
class BlockchainData {
    ...
    getBlockByHash(blockHash) {
        return this._blocksByHash.get(ethereumjs_util_1.bufferToHex(blockHash));
    }
    getBlockByTransactionHash(transactionHash) {
        return this._blocksByTransactions.get(ethereumjs_util_1.bufferToHex(transactionHash));
    }
    getTransaction(transactionHash) {
        return this._transactions.get(ethereumjs_util_1.bufferToHex(transactionHash));
    }
    getTransactionReceipt(transactionHash) {
        return this._transactionReceipts.get(ethereumjs_util_1.bufferToHex(transactionHash));
    }
    getTotalDifficulty(blockHash) {
        return this._totalDifficulty.get(ethereumjs_util_1.bufferToHex(blockHash));
    }
    getLogs(filterParams) {
        ...
        for (let i = filterParams.fromBlock; i.lte(filterParams.toBlock); i = i.addn(1)) {
            ...
            if (block === undefined ||
                !filter_1.bloomFilter(new bloom_1.default(block.header.bloom), filterParams.addresses, filterParams.normalizedTopics)) {
                continue;
            }
            for (const transaction of block.transactions) {
                const receipt = this.getTransactionReceipt(transaction.hash());
                if (receipt !== undefined) {
                    logs.push(...filter_1.filterLogs(receipt.logs, {
    ...
    addBlock(block, totalDifficulty) {
        ...
        for (const transaction of block.transactions) {
            const transactionHash = ethereumjs_util_1.bufferToHex(transaction.hash());
    ...
    addTransaction(transaction) {
        this._transactions.set(ethereumjs_util_1.bufferToHex(transaction.hash()), transaction);
    }
