...
task(
    'fund-on-demand-sub',
    'Funds a billing subscription for On-Demand Consumer contracts'
).addParam(
    "amount",
    "Amount to fund subscription in LINK"
).addParam(
    "subid",
    "Subscripton ID to fund"
    ...
    if (juelsAmount.gt(balance)) {
        throw Error(`Insufficent LINK balance. Trying to fund subscription with ${ethers.utils.formatEther(juelsAmount)} LINK, but wallet only has ${ethers.utils.formatEther(balance)}.`)
    }
