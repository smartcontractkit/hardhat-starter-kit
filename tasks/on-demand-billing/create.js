...
task(
    'create-on-demand-sub',
    'Creates a new billing subscription for On-Demand Consumer contracts'
).addOptionalParam(
    "amount",
    "Inital amount used to fund the subscription in LINK"
)
.addOptionalParam(
    "consumer",
    "Inital On-Demand client address to add to subscription"
    ...
    const createWaitBlockConfirmations = developmentChains.includes(network.name) || consumer || linkAmount
        ? 1
        : VERIFICATION_BLOCK_CONFIRMATIONS
    console.log(`Waiting ${createWaitBlockConfirmations} blocks for transaction ${createSubscriptionTx.hash} to be confirmed...`)
    ...
    if (linkAmount) {
        ...
        if (juelsAmount.gt(balance)) {
            throw Error(`Insufficent LINK balance. Trying to fund subscription with ${ethers.utils.formatEther(juelsAmount)} LINK, but only have ${ethers.utils.formatEther(balance)}.`)
        }
        ...
        const fundWaitBlockConfirmations = developmentChains.includes(network.name) || consumer
            ? 1
            : VERIFICATION_BLOCK_CONFIRMATIONS
