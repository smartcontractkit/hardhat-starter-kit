...
task(
    ...
    if (subInfo[1].toLowerCase() !== signerAddr.toLowerCase()) {
        throw Error(`Subscription ID ${subscriptionId} is owned by wallet address ${subInfo[1].toLowerCase()}, not ${signerAddr.toLowerCase()}`)
    }
    ...
    console.log(`Ownership transfer to ${newOwner} requested for subscription ${subscriptionId}.  The new owner must now accept the transfer request.`)
})