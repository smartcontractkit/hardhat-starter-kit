...
!developmentChains.includes(network.name)
    ...
    : describe('Price Consumer Unit Tests', async function () {
          ...
          async function deployPriceConsumerFixture() {
              ...
              const mockV3Aggregator = await mockV3AggregatorFactory.connect(deployer).deploy(DECIMALS, INITIAL_PRICE)

              const priceConsumerV3Factory = await ethers.getContractFactory('PriceConsumerV3')
              const priceConsumerV3 = await priceConsumerV3Factory.connect(deployer).deploy(mockV3Aggregator.address)
          ...
          describe('deployment', async function () {
              describe('success', async function () {
                  it('should set the aggregator addresses correctly', async () => {
                      const { priceConsumerV3, mockV3Aggregator } = await loadFixture(deployPriceConsumerFixture)
