...
async function deployOnDemandApiConsumer(chainId = network.config.chainId) {
    ...
    if (chainId == 31337) {
        ...
        mockEthLinkFeed = await mockEthLinkFactory.connect(deployer).deploy(0, ethers.BigNumber.from(5021530000000000))
        ethLinkFeedAddress = mockEthLinkFeed.address

        // Deploy Registry
        const registryFactory = await ethers.getContractFactory('OCR2DRRegistry')
        mockRegistry = await registryFactory.connect(deployer).deploy(linkTokenAddress, ethLinkFeedAddress)
        ...
        await mockOracle.setDONPublicKey(ethers.utils.toUtf8Bytes(networkConfig[chainId]['OCR2ODMockPublicKey']))
